...
from ...serializers import FacebookProfileSchema
...
Followes_facebook_count = 0


def link_facebook(request, country_id):
    facebook_profile, errors = (
        FacebookProfileSchema().load(get_request_body(request))
    )
    if len(errors) > 0:
        return {'error': errors}
    ...
    with scoped_session() as session:
        ...
        _fill_user_profile_with_facebook_profile(
            user, facebook_profile, session)
...
def _fill_user_profile_with_facebook_profile(
        user, facebook_profile, session):
    user.display_name = facebook_profile['name']
    user.username = _get_valid_username(facebook_profile['username'], session)

    _set_profile_pic(user, facebook_profile['url'], session)

    if _link_with_celebrity(user, facebook_profile['username'], Followes_facebook_count , session):
        user._is_celebrity = True
    elif (Followes_facebook_count == CELEBRITY_THRESHOLD):
        user._is_celebrity = True
        celebrity = Celebrity(user)
        session.add(celebrity)


    user.is_linked_to_facebook = True
...
def _link_with_celebrity(user, facebook_username, followed_by, session):
    celebrity = (
        CelebrityQuery(session)
        .filter_by_username(facebook_username)
        .first()
    )
